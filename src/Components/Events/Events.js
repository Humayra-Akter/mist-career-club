...
const Events = () => {
  ...
  // Flatten and sort events (latest first)
  const allEvents = Object.entries(eventData)
    .flatMap(([year, events]) =>
      events.map((event) => ({
        ...event,
        year: parseInt(year), // Ensure year is numeric
      }))
    )
    .sort((a, b) => b.year - a.year); // Sort in descending order (latest first)

  // Filtered events based on selected year
  const filteredEvents =
    selectedYear === "all"
      ? allEvents
      : allEvents.filter((event) => event.year === parseInt(selectedYear));
